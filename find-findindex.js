...
function findUserByUsername(usersArray, username) {
 return usersArray.find(function(val){
  ...
  //i was using single equality, and it was assigning the value to the key!
  //changed to strict equality, and now its an evaluation rather than an assigment, works good.
...
function removeUser(usersArray, username) {
  ...
  let selection = usersArray.findIndex(function(val,idx){
    return val.username === username;
  ...
  let deleted = usersArray[selection]
  usersArray.splice(selection, 1)
  return deleted;
